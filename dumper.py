...
class MySQLDumper(BaseDumper):
    ...
    def __init__(self, config):
        ...
        try:
            ...
            self.sql = config.get('Options', 'sql')
            self.path = config.get('Options', 'path')
            self.index = config.get('Options', 'index')
    ...
    def dump(self):
        ...
        if ensure_dir(self.path):
            print "\033[1;33m[Created]\033[1;m %s" % self.path
        ...
        for result in results:
            ...
            record = "%s/%s" % (self.path, result[self.index])
